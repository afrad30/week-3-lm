...
mh = function(n, ybar, n_iter, mu_init, cand_sd) {
  ...
  accpt = 0
  mu_now = mu_init
  #mu now for denominator
  lg_now = lg(mu=mu_now, n=n, ybar=ybar)# means g(theta*)
  # lg_now also for demoninator
  
  ## step 2, iterate
  for (i in 1:n_iter) {
    ## step 2a
    mu_cand = rnorm(n=1, mean=mu_now, sd=cand_sd) #t distribution for draw a candidate, present value
    
    ## step 2b
    lg_cand = lg(mu=mu_cand, n=n, ybar=ybar) # putting mu value in Normal distribution, evaluate log of g with the candidate
